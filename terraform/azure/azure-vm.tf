...
jobs:
  ...
  terraform-deploy:
    # This job will only run if the 'code-scan' job is successful.
    needs: code-scan
    runs-on: ubuntu-latest
    steps:
      - name: Checkout Source Code
      ...
      # This new step generates a detailed plan and saves it to a file.
      - name: Terraform Plan
        run: terraform plan -input=false -out=tfplan -var="github_username=${{ github.repository }}" -var-file="terraform.tfvars" -var="azure_subscription_id=${{ env.AZURE_SUBSCRIPTION_ID }}"
        working-directory: 'terraform/azure/'

      # The 'apply' command now uses the plan file from the previous step.
